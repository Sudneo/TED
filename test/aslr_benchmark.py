...
class aslr_scan:
    ...
    def hard_scan_native(self):
        ...
        #client = docker.DockerClient(base_url='unix:///var/run/docker.sock')
        for i in range(10):
            output = subprocess.check_output("/home/daniele/University/thesis/src/Docker/ASLR/get_addresses",shell=True)
            lines = output.split("\n")
            env_addr=lines[0].split("=")[1]
            stack_addr=lines[1].split("=")[1]
            heap_addr=lines[2].split("=")[1]
    ...
    def soft_scan_native(self):
        ...
        3 means ASLR is enabled and further patches are applied
        """
        output = subprocess.check_output("sysctl kernel.randomize_va_space",shell=True)
        splitted = output.split ("=")
        soft_check = {"ASLR_soft": splitted[1].rstrip("\n").lstrip(" ")}
        return soft_check
    ...
    def scan_native(self):
        ...
        :return: The combiantion of the report for the soft and the hard scan
        """
        hard_report = self.hard_scan_native()
        soft_report = self.soft_scan_native()
        list_value = []
        list_value.append(hard_report)
        list_value.append(soft_report)
        result = {'ASLR': list_value}
